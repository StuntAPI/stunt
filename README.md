...
> `winget install StuntAPI.Stunt` (no clone).
...
passes an adversarial input-safety sweep (garbage params, null/malformed bodies, tampered
cursors — never a 5xx) plus coverage-guided fuzzing of the engine's parsers
(`just fuzz` for longer rounds). Highlights:
