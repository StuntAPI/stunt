...
// A DISCLAIMER is generated when the adapter name uses the branded
// "-style" suffix convention.
func scaffoldFiles(name string) map[string]string {
	...
	// Branded adapters (the "-style" naming convention) get a DISCLAIMER
	// reminding users the adapter is unofficial and uses synthetic data.
	if strings.HasSuffix(name, "-style") {
		files["DISCLAIMER"] = disclaimer(display)
	}
	return files
}

// disclaimer renders a DISCLAIMER for a branded adapter, with {PROVIDER}
// replaced by the humanized display name (with any trailing " Style"
// trimmed, so "Reddit Style" → "Reddit").
func disclaimer(display string) string {
	provider := display
	if i := strings.LastIndex(strings.ToLower(provider), " style"); i >= 0 {
		provider = provider[:i]
	}
	return strings.ReplaceAll(disclaimerTemplate, "{PROVIDER}", provider)
}

const disclaimerTemplate = `# DISCLAIMER

This adapter is **not affiliated with, endorsed by, or sponsored by** {PROVIDER}.
"{PROVIDER}" and related marks are trademarks of their respective owners.

This is a **local development and testing simulator** provided by the ` + "`stunt`" + ` project.

- It **does not** call the real {PROVIDER} API.
- It runs entirely on your local machine and returns **synthetic, fake data only**.
- It contains **no real {PROVIDER} data, no recorded responses, and no proprietary
  documentation**. All fixtures and templates are generated by fakers and pass
  ` + "`stunt adapter lint`" + `.
- It reproduces the *structure* of a {PROVIDER}-style API solely so you can develop and
