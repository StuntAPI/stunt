...
- **Scoping** — everything lives under `/entities/{company_id}`. A company id
...
| taxes | `GET /entities/{c}/taxes` (the standard Italian VAT bands) |
| cashbook | `GET /entities/{c}/cashbook/{year}/{month}` |
...
`GET /entities/{c}/received_documents/info` returns the categories in use —
