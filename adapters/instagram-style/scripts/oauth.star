# OAuth2 handlers — Meta Instagram authorization code flow.
#
# GET  /oauth/authorize   -> 302 redirect with code+state
# POST /oauth/access_token -> { access_token, user_id }
#
...
def on_authorize(req):
    ...
    code_seq = store_kv_incr("instagram", "code_seq")
...
def _mint_user():
    seq = store_kv_incr("instagram", "user_seq")
    uid = "ig_" + str(seq)
    return {
        "ig_user_id": uid,
...
def _mint_token(user):
    token_seq = store_kv_incr("instagram", "token_seq")
